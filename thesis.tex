...
\begin{definition}[Solution of a Gaussian stochastic partial differential equation]
Let $H$ be a Hilbert space, let $\c{L} : H \-> G$ be a bounded linear operator, and let $\c{W}$ be a generalized Gaussian field on $G$.
...
\begin{result}[Solution criteria for Gaussian stochastic partial differential equations]
...
This can be achieved by choosing it to be a reproducing kernel Hilbert space and building the classical Gaussian process from pointwise evaluation functionals.
...
Here, we adapt ideas of \textcite{whittle63} and \textcite{lindgren11} to the function-space setting.
To start, we define the right-hand-side.

\begin{definition}[White noise]
Define the \emph{white noise process} $\c{W} : \Omega \x L^2(X) \-> \R$ to be a centered generalized Gaussian field with covariance operator $I : L^2(X) \-> L^2(X)$.
...
This stochastic process should \emph{not} be confused with a random variable.
Though Kolmogorov's Extension Theorem also implies there is a random variable $\c{W} : \Omega \-> \R^{L^2(X)}$ whose finite-dimensional marginals coincide with $\c{W} : \Omega \x L^2(X) \-> \R$, this is next-to-useless because $\R^{L^2(X)}$ is highly irregular and we can say little about which subspace $\c{W}$ concentrates on without additional considersations that we rather conveniently avoid.
...
\[
H^k(X) = \cbr{f \in D'(X) : f = (\id + \lap_g)^{-k/2} g : g \in L^2(X)}
\]
...
This enables us to plug pointwise evaluation functionals into the obtained generalized Gaussian field, obtaining Riemannian Gaussian processes in the classical sense---the objects we sought to construct in the first place.

To carry this program out, we must prove a set of auxillary technical lemmas to ensure all of the considered ideas glue together with one another in a well-defined manner.
