...
In \Cref{ch:noneuclidean}, we present Matérn Gaussian processes in non-Euclidean settings: this work is published in \textcite{borovitskiy20,borovitskiy21} and under review in \textcite{hutchinson21}.
...
Let $\c{S}$ be a \emph{semi-algebra of sets}, which is defined as a family of sets satisfying the following.
...
\0 
Then any countably additive function $\mu : \c{S} \-> [0,1]$ satisfying $\mu(\emptyset) = 0$ extends uniquely to a measure on the smallest $\sigma$-algebra containing $\c{S}$.
\end{lemma}
...
We claim every function $\mu : \c{S} \-> [0,1]$ satisfying the measure axioms extends uniquely to a function on $a(\c{S})$.
...
We have thus obtained a uniquely defined countably additive function $\mu^{(a)} : a(\c{S}) \-> [0,1]$ defined on an algebra of sets $a(\c{S})$ which extends $\mu$.
This function satisfies the assumptions of Carathéodory's Extension Theorem---see \textcite[Theorem A1.1]{kallenberg06} or \textcite[Theorem 3.1]{billingsley08}---applying this result gives the claim, where we note that uniqueness follows since the range of $\mu^{(a)}$ is $[0,1]$.
\end{proof}

\begin{lemma}
\label{lem:cyl-prod}
Let $\pi : \c{A} \x \c{B} \-> [0,1]$ be a countably additive function with $\pi(\emptyset) = 0$.
...
This level of generality will be important for two reasons: (i) to provide an appropriate formalism for studying stochastic partial differential equations, and (ii) to develop a coordinate-free notion of Gaussian random vectors that will be useful in the differential-geometric setting.
\0 
...
\begin{definition}[Gaussian process (in the sense of duality)]
...
From this, one might be tempted to conclude that there is nothing more to say conditioning multivariate Gaussians---this, however, would miss an alternative view.
...
Vector fields on manifolds exhibit certain behaviors not present in the Euclidean case---we illustrate one of the most important cases below.
