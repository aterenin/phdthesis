...
\]
where $s_{t+1} \~ p(s_t, a_t)$, $a_t \~ \pi(s_t)$, and $r_t \~ r(s_t,a_t)$.
\end{definition}

Finding an optimal policy therefore amounts to selecting the best possible actions to maximize rewards up to the given time horizon.
It is therefore of key interest to find an optimal policy, if one exists.
This problem has different names, depending on the setting one considers---some particularly important settings are given as follows.

\1 If $p$ and $r$ are known, it is known as an \emph{optimal control} problem.
\2 Otherwise, it is known as a \emph{decision} problem.
\0 

These problem classes differ fundamentally from one another. 
...
Here, one generally proceeds by describing equations that $V^{(\pi)}$ and $\pi$ satisfy, and developing schemes for solving them.


Due to the rewards being \emph{unknown}, they cannot simply be maximized and their expectation must be \emph{learned}.
...
Explore-exploit tradeoffs caused by lack of knowledge of state transitions, or of rewards, fundamentally differentiate optimal control problems from all others in the list.
In particular, they make it impossible to simply \emph{solve} the respective problem by maximizing $V^{(\pi)}$, and one must develop other solution concepts.
...
Instead, the goal is to find a way of updating the policy based on observed data so as to limit growth of regret.
...
The obviously-bad algorithm which chooses the exact same action over and over again incurs at most linear regret.
...
We will distinguish between two key kinds of decision problems.
...
Multi-armed bandits possess no variable state, and thus only require one to determine which actions are optimal.
...
Doing this yields an approach called \emph{Thompson sampling}, which can be shown optimal in many settings.
