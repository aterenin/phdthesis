...
In \Cref{ch:noneuclidean}, we present Matérn Gaussian processes in non-Euclidean settings: this work is published in \textcite{borovitskiy20,borovitskiy21,jacquier21} and under review in \textcite{hutchinson21}.
...
We use \textcite{sutton18,bertsekas19} as reference.
...
Algorithms within this class date back to \textcite{lai85,agrawal95}: we focus on the variant analyzed by \textcite{auer02}.
...
One can show this algorithm, illustrated in \Cref{fig:bb-ucb}, achieves the same regret as Hoeffding--UCB: the proof is similar, but instead uses the empirical Bernstein inequality, and is significantly more messy owing to the complicated form of the posterior mean and standard deviation.
...
We have made no attempt to optimize the bound, and significant improvements are possible, particularly when considering variations of the UCB algorithm---see \textcite{lattimore20} for state-of-the-art results.
...
Since the covariance form is a map $k : V^* \x V^* \-> \R$, it gives rise to an operator $\c{K} : V^* \-> V^{**}$, called the \emphmarginnote{covariance operator}, which in the case that $V$ is reflexive becomes an operator $\c{K} : V^* \-> V$ via composition with the isomorphism given by reflexivity.
...
Additionally, \Cref{thm:mvn-pw} is reasonably well-known in geostatistics, and has been used or described by \textcite{journel78,defouquet94,emery07,oliver96} in addition to the above authors.
...
Such Gaussian processes are also of direct interest in the non-Euclidean settings considered in \Cref{ch:noneuclidean}.
...
This idea underlies \emph{inducing point} approximations, including those proposed by \textcite{snelson06,titsias09,opper09,hensman13}, as well as many others.

To ensure the approximation is expressive enough, we can either (i) make $\v\mu$ random with a learnable mean and covariance, or (ii) introduce a learned noise covariance $\m\Lambda$ rather than reusing the one from the original model.
...
Results can be seen in \Cref{fig:parallel-ts}, where we plot regret curves 
...
\begin{proof}
See \textcite[page 139]{chavel84} or \textcite[Theorem 44]{canzani13}.
\end{proof}
...
For this, \textcite{neiswanger21} further propose \emph{InfoBAX}, an information-theoretic acquisition function constructed directly from the algorithm being considered.
...
Specifically, one runs the underlying algorithm on posterior random functions, producing a set of \emph{execution traces} of points queried by the algorithm.
...
Bayesian algorithm execution enables such methods to be considered for settings beyond Bayesian optimization and model-based reinforcement learning. 
...
From a theoretical point of view, one of the first questions to be addressed in understanding Bayesian algorothm execution is to understand what is the appropriate notion of \emph{regret} for this setting.
...
Multi-objective Bayesian optimization, where the goal is finding a Pareto frontier, rather than obtaining a global minima, gives one concrete algorithm class to consider when looking for such applications.
...
Another promising direction is exploring the connections between Gaussian processes and numerical analysis.
Here, one can study use of iterative solvers, such as the conjugate gradient method, in combination with pathwise conditioning and inducing points.
...
The computational costs of an iterative solver depend on how many iterations are required for convergence.
This, in turn, varies according to the condition number of the linear system being solved.
...
Matrix square roots generally require further considerations than simply solving linear systems and are less convenient.
...
Understanding how to efficiently compute such terms accurately is therefore a key issue to address in tackling these questions.

In the partial differential equation and mathematical physics communities, iterative solvers for linear systems constructed using finite element methods achieve state-of-the-art performance for many problems.
...
A key lesson from numerical analysis is that, in order to solve a linear, the most important step is often the construction of a \emph{preconditioner} to partially solve the linear system before an iterative method is used.
...
Multi-grid methods provide a broad framework for constructing preconditioners which achieve state-of-the-art performance for many partial differential equations. 
...
Adapting such ideas to the Gaussian process setting is a particularly promising avenue for improvement.
...
Multi-scale methods suggest that right kind of approximation for this regime should involve replacing long-range effects with averaged, sparsified versions thereof.
Understanding how construct such a variational approximation is therefore also a promising avenue for future work.
...
For example, on the sphere, the eigenfunctions of the Laplace--Beltrami operator are the spherical harmonics.
The spherical harmonics, in turn, can be re-expressed in terms of the \emph{Gegenbauer polynomials}, which are a set of polynomials on $[-1,1]$ which are orthogonal with respect to a certain weight function.
...
This is a substantial simplification compared to the general case, and one can use it to pre-compute the kernel for various values of the geodesic distance, and interpolate the resulting values, rather than evaluate an infinite series each time the kernel needs to be computed.
...
Since some Lie groups are non-compact, studying kernels on Lie groups and related spaces such as certain matrix manifolds is a promising avenue towards developing a general theory of Riemannian kernels which does not require compactness.
Here, one would need to appeal to a more general spectral theory than that built using Sturm--Liouville decompositions.
Generalizations are available, but they are more mathematically sophisticated, requiring additinal machinery such as integration over projection-valued measures.
...
Spaces like this may be important in potential applications in areas like physics and engineering. 
Studying heat kernels, which have been considered in settings as general as metric-measure spaces, gives hints about the total scope which may be possible.
