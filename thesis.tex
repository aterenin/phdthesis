...
Bayesian learning using Gaussian processes provides a foundational platform for making decisions in a manner that balances what is known with what could be learned by gathering data. In this dissertation, we develop techniques for broadening the applicability of Gaussian processes in two ways.
...
These improves efficiency and makes it simpler to deploy Gaussian process models within decision-making settings.
...
This enables such models to be trained using standard computational techniques well-studied in the Euclidean case.
Using these ideas as a key building block, we describe a formalism for defining and constructing vector-valued Gaussian processes on Riemannian manifolds. 
In total, these contributions make Gaussian processes easier to work with, and allow them to effectively used within a wider class of domains and decision-making settings.
...
Moreover, \Cref{thm:mvn-pw} is reasonably well-known in geostatistics, and has been used or described by \textcite{journel78,defouquet94,emery07,oliver96} in addition to the above.
...
Observe that approximate pathwise sampling involves only matrix inverses, and does not require computation of matrix square roots.
This makes applying Gaussian-process-specific iterative schemes for solving linear settings, such as those proposed by \textcite{gardner18,pleiss18,meanti20}, particularly straightforward.
Note, however, that training may require computation of log-determinant terms: \textcite{dong17,pleiss20} develop techniques which can be used to handle this.
...
Moreover, acquisition functions can possess properties which make them easy to optimize: see \textcite{wilson18} for details.
...
Such Gaussian processes are also of direct interest in non-Euclidean settings considered in \Cref{ch:noneuclidean}.
...
The inducing point approximation presented above is particularly interesting because the matrix $\m{K}_{\v{x}\v{x}} + \m\Lambda$ which needs to be inverted is generally better-conditioned compared to that present in other approximations, owing to the presence of $\m\Lambda$, whose learned values tend to be large.
This makes this variational family a particularly attractive choice for use in combination with iterative solvers \cite{dong17,gardner18,pleiss18,meanti20,pleiss20}.
...
Future work: pathwise with iterative solvers

Future work: infoBAX

Future work: geometric kernels on more spaces

Future work: applications of geometric methods
