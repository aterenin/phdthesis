...
A $\sigma$-algebra is a set of subsets of $Y$ closed under unions, intersections, complements, and set-theoretic monotone limits.
...
This can be thought of as a map that takes a true/false question, and assigns a number indicating how close to true or false its answer is, according to the measure.
A \emph{probability space} is a measurable space equipped with a probability measure.
The set $\Omega$ should be viewed as a space of random numbers, with a random number generator described by the measure $\P$.
...
A random variable, then, maps random numbers $\omega\in\Omega$ into some space of interest $Y$.
The \emph{distribution} of a random variable is defined as the \emph{pushforward measure} $\pi_y(A_y) = (y_* \P)(A_y) = \P(y^{-1}(A_y))$ for $A_y\in\c{Y}$.
The probability an event $A_y$, then, is determined by measuring the probability of random numbers under which they occur---measurability guarantees this is always possible.
...
For an arbitrary sequence of points $x_1,..,x_n$, define the likelihood
\[
...
.
\]
Here, the data is $Y = \R^n$, and our quantity of interest is the actual \emph{function} $f$, which is now defined on an infinite-dimensional vector space, say, for instance, the space of continuous functions $C^0(X;\R)$.

It is now clear why we bothered with setting up a dense, abstract formalism for Bayesian learning in general measure spaces: this formalism is rich and powerful enough to enable us to properly define priors $\pi_f$ on spaces like this---we explore these in the sequel.
...
\[
BR(T) = \E_f R(T) = \E \sum_{t=1}^T f(\omega,x_t) - f(\omega,x^*)    
\]
